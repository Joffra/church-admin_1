...
// Binary status toggle: actif <-> fermé
async function toggleStatus(church) {
  const newStatus = church.status === 'active' || church.status === 'actif' ? 'closed' : 'active'
  togglingId.value = church.id
  try {
    await ChurchesAPI.update(church.id, { status: newStatus })
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-5 py-3.5 text-ink-dark/60">{{ church.address || '—' }}</td>
