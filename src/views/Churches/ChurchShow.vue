...
async function toggleStatus() {
  ...
  const newStatus = church.value.status === 'active' || church.value.status === 'actif' ? 'closed' : 'active'
  togglingStatus.value = true
  try {
    await ChurchesAPI.update(church.value.id, { status: newStatus })
    church.value.status = newStatus
  } catch (e) {
    error.value = 'Impossible de changer le statut.'
  } finally {
    togglingStatus.value = false
  }
}

// Archive = soft delete (deleted_at). Record stays in the database for
// historical/audit purposes but is excluded from default queries.
    ...
    <div v-else-if="church">
      <div class="mb-6 flex flex-wrap items-start justify-between gap-4 border-b border-rule pb-6">
        <div>
          ...
          <h1 class="mt-1 font-display text-3xl text-ink-dark">{{ church.name }}</h1>
          <button
            @click="toggleStatus"
      ...
      <dl class="grid grid-cols-1 gap-5 rounded-lg border border-rule bg-white p-6 sm:grid-cols-2">
        <div>
        ...
        <div>
          <dt class="text-[11px] font-semibold uppercase tracking-wide text-ink-dark/40">Ville</dt>
          <dd class="mt-1 text-sm text-ink-dark">{{ church.city || '—' }}</dd>
        </div>
        <div>
          <dt class="text-[11px] font-semibold uppercase tracking-wide text-ink-dark/40">Téléphone</dt>
          <dd class="mt-1 text-sm text-ink-dark">{{ church.phone || '—' }}</dd>
        </div>
        <div>
          <dt class="text-[11px] font-semibold uppercase tracking-wide text-ink-dark/40">Email</dt>
          <dd class="mt-1 text-sm text-ink-dark">{{ church.email || '—' }}</dd>
        </div>
      </dl>
    </div>
